...
local LibCasterino = LibStub("LibClassicCasterino",true)
...
function CarboniteInfo:OnInitialize()
	...
	UnitCastingInfo = function(unit)
		return LibCasterino:UnitCastingInfo(unit)
	end
...
function Nx.Info:CalcCast (target, w, h)
	...
	local spell, rank, name, icon, startTime, endTime = CastingInfo ()

	if not name then
		spell, rank, name, icon, startTime, endTime = ChannelInfo ()
	end
...
function Nx.Info:CalcBGStats()
	if Nx.InBG then
		local cb = Nx.Combat
		return "|cffa0a0a0", format ("%d %d %d", cb.KBs, cb.Deaths, cb.HKs)
	end
end
